...
int main(void)
{
    ...
    while (!WindowShouldClose())
    {
        if (gameOver || gameWon) {
            ...
            if (gameWon) DrawText("CONGRATULATIONS! YOU WON!", screenWidth/2 - 170, screenHeight/2 - 20, 20, GREEN);
            DrawText("Press [R] to Restart", screenWidth/2 - 110, screenHeight/2 + 20, 20, DARKGRAY);
            if (gameOver) DrawText("Game Over! You Lost!", screenWidth/2 - 70, screenHeight/2 - 60, 30, RED);
            EndDrawing();
            continue;
        }
        ...
        if (!anyBallActive) {
            ...
            if (lives <= 0) {
                ...
                balls[0] = (Ball){10.0, (Vector2){paddle.x + paddle.width / 2, paddle.y - 20}, BLACK, 5, -5, true};
                for (int i = 1; i < MAX_BALLS; i++) balls[i].active = false;
            }
        }
        ...
        DrawText(TextFormat("Lives: %d", lives), screenWidth - 100, 40, 20, MAROON);
